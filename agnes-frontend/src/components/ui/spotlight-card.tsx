...
const GlowCard: React.FC<GlowCardProps> = ({
  ...
  const inlineStyles: React.CSSProperties & Record<string, string | number> = {
    ...
    backgroundImage: `radial-gradient(
      ...
      calc(var(--x, 0) * 1px) calc(var(--y, 0) * 1px),
      hsl(var(--hue, 210) calc(var(--saturation, 60) * 1%) calc(var(--lightness, 50) * 1%) / var(--bg-spot-opacity, 0.04)), transparent
    )`,
  ...
  return (
    <div
      ...
      data-glow
      style={inlineStyles}
      className={`${customSize ? '' : sizeMap[size]} rounded-2xl relative shadow-[0_1rem_2rem_-1rem_black] ${className}`}
    >
